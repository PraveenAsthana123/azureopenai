# ==============================================================================
# Automated Call Center Copilot - Terraform Infrastructure
# ==============================================================================
...
terraform {
  ...
  backend "azurerm" {
    ...
    key                  = "call-center-copilot.tfstate"
  }
}
...
resource "azurerm_cosmosdb_sql_container" "call_summaries" {
  name                = "callSummaries"
  resource_group_name = azurerm_resource_group.main.name
  account_name        = azurerm_cosmosdb_account.main.name
  database_name       = azurerm_cosmosdb_sql_database.main.name
...
# ==============================================================================
# Azure Functions (Call Center Copilot)
# ==============================================================================
...
resource "azurerm_linux_function_app" "copilot" {
  name                       = "func-callcenter-${local.resource_prefix}"
  location                   = azurerm_resource_group.main.location
  resource_group_name        = azurerm_resource_group.main.name
  ...
  app_settings = {
    ...
    "LANGUAGE_ENDPOINT"        = azurerm_cognitive_account.speech.endpoint
    "STORAGE_ACCOUNT_URL"      = azurerm_storage_account.functions.primary_blob_endpoint
...
resource "azurerm_role_assignment" "function_openai" {
  ...
  principal_id         = azurerm_linux_function_app.copilot.identity[0].principal_id
}
...
output "function_app_url" {
  value = "https://${azurerm_linux_function_app.copilot.default_hostname}"
}
